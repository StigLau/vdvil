...
public class MixPersistenceStorageTest {
    ...
    public void play() throws IOException {
        File theFile = new File("/tmp/" + getClass().getSimpleName() + new Random().nextInt() + ".wav");
        new PreconfiguredWavSerializer(theFile).init(composition).playUntilEnd();
        String fileCheckSum = DigestUtils.md5Hex(new FileInputStream(theFile));
