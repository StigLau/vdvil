...
public class SplineDisplay extends EquationDisplay {
  ...
  public void setControl1(Point2D control1) {
    support.firePropertyChange("control1", (Point2D) this.control1.clone(), (Point2D) control1.clone());
    this.control1 = (Point2D) control1.clone();
    repaint();
  }

  public void setControl2(Point2D control2) {
    support.firePropertyChange("control2", (Point2D) this.control2.clone(), (Point2D) control2.clone());
  ...
  synchronized void saveAsTemplate(OutputStream out) {
    ...
    BufferedImage subImage = image.getSubimage((int) xPositionToPixel(0.0), (int) yPositionToPixel(1.0),
        (int) (xPositionToPixel(1.0) - xPositionToPixel(0.0)) + 1, (int) (yPositionToPixel(0.0) - yPositionToPixel(1.0)) + 1);
  ...
  private void paintControlPoint(Graphics2D g2, Point2D control) {
    ...
    Ellipse2D inner = new Ellipse2D.Double(origin_x + 2.0 - CONTROL_POINT_SIZE / 2.0, origin_y + 2.0 - CONTROL_POINT_SIZE / 2.0,
        8.0, 8.0);
  ...
  private void paintSpline(Graphics2D g2) {
    CubicCurve2D spline = new CubicCurve2D.Double(xPositionToPixel(0.0), yPositionToPixel(0.0), xPositionToPixel(control1.getX()),
        yPositionToPixel(control1.getY()), xPositionToPixel(control2.getX()), yPositionToPixel(control2.getY()),
  ...
  private class ControlPointsHandler extends MouseMotionAdapter {
    ...
    public void mouseDragged(MouseEvent e) {
      ...
      support.firePropertyChange("control" + (selected == control1 ? "1" : "2"), selectedCopy, (Point2D) selected.clone());
